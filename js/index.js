...
class Template {
    static searchHistoryOnSearchBox(text) {
        return `<button class="button button__icon-text button__background-hover-normal search__history__option not-scale"
                    ...
                    <span class="button button__only-icon button__color-hover-red not-scale search__history__delete"
                          onclick="search.historyClear(event, '${text}')">
    ...
    static engineOnEngine(name, website, index) {
        return `<label class="setting__option only-operation radio__container style-engine engine__custom">
                    <input type="radio" name="engine-default" onchange="settings.setEngineDefault(${index})">
                    ...
                    <span class="button button__only-icon button__color-hover-red not-scale default__engine__delete"
                          onclick="settings.delEngine('${name}')">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                             viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                             stroke-linecap="round" stroke-linejoin="round" class="feather feather-x">
                            <line x1="18" y1="6" x2="6" y2="18"></line>
    ...
    static toast(id, color, svg, text, time) {
        ...
        toast.innerHTML = `<div class="toast__context">
                            ...
                            <button class="button button__only-icon button__color-hover-red" onclick="toast.removeToast(this.parentElement)">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none"
                                     stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                                     class="feather feather-x">
                                    <line x1="18" y1="6" x2="6" y2="18"></line>
...
class Settings {
    ...
    constructor() {
        this.#elements = {
            ...
            other: {
                enginePresets: app.querySelectorAll("#popup__engine input[type=radio][name='engine-default']")
            }
        };
    ...
    initValue() {
        ...
        // init engine
        this.#engine[store.value.website.default].checked = true;
        this.reloadEngine();
    ...
    reloadEngine() {
        this.#elements.container.engineOnSearch.querySelectorAll("button.engine__custom").forEach(element => element.remove());
        this.#elements.container.engineOnEngine.querySelectorAll("label.engine__custom").forEach(element => element.remove());
        const websites = store.value.website.custom;

        Object.keys(websites).forEach((name, index) => {
            this.#elements.container.engineOnSearch.innerHTML += Template.engineOnSearchBox(name, websites[name]);
            this.#elements.container.engineOnEngine.appendChild(
                Util.parseHTMLStringToElement(
                    Template.engineOnEngine(name, websites[name], index + this.#elements.other.enginePresets.length)
                )
            );
        });
    }
    ...
    initEventListener() {
        this.#elements.other.enginePresets.forEach((engine, index) => {
...
class Search {
    ...
    reloadHistory() {
        ...
        histories.forEach(history => {
            this.#historyOnSearchBox.innerHTML += Template.searchHistoryOnSearchBox(history);
            this.#historyOnRoutine.innerHTML += Template.searchHistoryOnRoutine(history);
        });
    }
